...
from luq.methods.top_k_gap import TopKGapEstimator
...
def test_top_k_gap_estimator():
    samples = gen_samples(24)  # TODO parameterize with num samples
    estimator = TopKGapEstimator()
    # Calculate entropy

    for k in range(2, len(samples)):
        uncertainty = estimator.estimate_uncertainty(samples, k=k)
        assert isinstance(uncertainty, float)
        assert np.isclose(uncertainty, 1, atol=1e-9)


def test_top_k_gap_estimator_one_confident_answer():
